...
describe('Pagination Component', () => {
        ...
        it('renders all sizes correctly', async () => {
            await testAllSizes(Pagination, { totalPages: 5 }, (size) => {
                ...
                buttons.forEach((button) => {
                    expect(button).toHaveClass(size === 'sm' ? 'h-8 min-w-8' : size === 'md' ? 'h-10 min-w-10' : 'h-12 min-w-12');
                });
            });
        });
    });
    describe('Page Numbers', () => {
        ...
        it('shows ellipsis for large page counts', () => {
            ...
            const ellipsisElements = Array.from(ellipsis).filter((el) => el.textContent?.includes('...'));
