...
<script lang="ts">
	...
	import { threlteConfig, saveThrelteConfig } from '../stores/threlte.js';
	...
	$effect(() => {
		return toEffect(threlteConfig, (config) => {
			...
			console.log('Threlte config updated:', config);
	...
	useDebouncedEffect(() => {
		if (element && isVisible) {
			console.log('Element became visible with debounce');
		}
	}, 300);
