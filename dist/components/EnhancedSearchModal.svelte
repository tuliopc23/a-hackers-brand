					...
					<div class="flex flex-wrap gap-2">
						{#each searchIndex.getAvailableCategories() as category (category.id || category)}
							<button
								...
								onclick={() => selectCategory(category)} onkeydown={(e) => e.key === "Enter" && selectCategory(category)} 
								...
								<div class="max-w-md mx-auto">
									...
									<div class="space-y-1">
										{#each suggestions().slice(0, 5) as suggestion, index}
											<button
												...
												onclick={() => selectSuggestion(index)} onkeydown={(e) => e.key === "Enter" && selectSuggestion(index)} 
									...
									{#if searchIndex?.getRecentSearches().length > 0}
										<button
											...
											onclick={clearRecentSearches} onkeydown={(e) => e.key === "Enter" && clearRecentSearches(e)}
										>
											Clear recent searches
										</button>
									{/if}
