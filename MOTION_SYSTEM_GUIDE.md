...
<div in:liquidBlur={{ duration: 300, blur: 'md' }}>Content with liquid entrance</div>
```
...
```svelte
<div in:glassFade={{ direction: 'up', duration: 400 }}>Glass fade content</div>
```
...
```svelte
<button use:springPop={{ scale: 1.05, duration: 200 }}> Interactive element </button>
```
...
<button use:magneticHover={{ strength: 0.3, distance: 100 }}> Magnetic button </button>
```
...
```svelte
<div use:jellyHover={{ intensity: 0.15, speed: 400 }}>Jelly element</div>
```
...
```svelte
<div use:breathing={{ enabled: true, intensity: 0.02, speed: 4000 }}>Breathing element</div>
```
...
<div in:liquidBlur={useBlur ? { blur: 'md' } : undefined} in:glassFade={!useBlur ? { direction: 'up' } : undefined}>
...
<div use:trapFocus={modalOpen}>Modal content with focus trap</div>
```

### Screen Reader Announcements

```svelte
<div in:liquidBlur aria-live="polite" aria-label="Content loaded">Dynamic content</div>
```
...
<button use:breathing={{ enabled: loading }} use:springPop={{ scale: loading ? 1.02 : 1.05 }}>
...
<div in:springPop={success ? { scale: 1.1, duration: 400 } : undefined} class:success>Form submitted!</div>
```
