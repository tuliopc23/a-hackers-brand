import { U as shallowRef, S as watchEffect, W as readonly, B as watch, X as customRef, I as getCurrentScope, J as onScopeDispose, E as effectScope, O as getCurrentInstance, y as hasInjectionContext, z as inject, T as provide, G as isRef, C as unref, Y as toValue$1, L as computed, M as reactive, K as toRefs$1, N as toRef$1, u as ref, Z as onBeforeMount, b as nextTick, _ as onBeforeUnmount, $ as onMounted, P as onUnmounted, H as isReactive, d as defineComponent, V as shallowReactive, x as h, F as Fragment, a0 as TransitionGroup, a1 as onUpdated, A as toRaw, D as markRaw, a2 as isReadonly, a3 as onBeforeUpdate } from './vue.runtime.esm-bundler.js';
...
function useAnimate(target, keyframes, options) {
  ...
  function update(init) {
    ...
    if (!isSupported.value || !el)
      return;
    if (!animate.value)
      animate.value = el.animate(toValue$1(keyframes), animateOptions);
    if (persist)
...
function useDraggable(target, options = {}) {
  ...
  const position = ref(
    (_a = toValue$1(initialValue)) != null ? _a : { x: 0, y: 0 }
  );
  ...
  const handleEvent = (e) => {
    if (toValue$1(preventDefault))
      e.preventDefault();
    if (toValue$1(stopPropagation))
      e.stopPropagation();
  };
  const start = (e) => {
    var _a2;
    if (!toValue$1(buttons).includes(e.button))
      return;
    if (toValue$1(options.disabled) || !filterEvent(e))
      return;
    if (toValue$1(exact) && e.target !== toValue$1(target))
      return;
...
function useFileSystemAccess(options = {}) {
  ...
  async function open(_options = {}) {
    if (!isSupported.value)
      return;
    const [handle] = await window.showOpenFilePicker({ ...toValue$1(options), ..._options });
...
function useMediaControls(target, options = {}) {
  ...
  useEventListener(
    target,
    "timeupdate",
    () => ignoreCurrentTimeUpdates(() => currentTime.value = toValue$1(target).currentTime),
    listenerOptions
  );
  useEventListener(
    target,
    "durationchange",
    () => duration.value = toValue$1(target).duration,
    listenerOptions
  );
  useEventListener(
    target,
    "progress",
    () => buffered.value = timeRangeToArray(toValue$1(target).buffered),
    listenerOptions
  );
  useEventListener(
    target,
  ...
  useEventListener(
    target,
    "ratechange",
    () => rate.value = toValue$1(target).playbackRate,
    listenerOptions
  );
  useEventListener(
    target,
...
function useTransition(source, options = {}) {
  ...
  watch(sourceVal, async (to) => {
    var _a, _b;
    if (toValue$1(options.disabled))
      return;
    const id = ++currentId;
    if (options.delay)
      await promiseTimeout(toValue$1(options.delay));
    if (id !== currentId)
      return;
