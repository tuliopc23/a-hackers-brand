<script lang="ts">import { isInstanceOf, T, useParent, useThrelte } from '@threlte/core';
import { BackSide, Color, Group, InstancedMesh, Mesh, ShaderMaterial, SkinnedMesh, Vector2 } from 'three';
import { toCreasedNormals } from 'three/examples/jsm/utils/BufferGeometryUtils.js';
import { fragmentShader, vertexShader } from './shaders';
import { fromStore } from 'svelte/store';
let { color = 'black', screenspace = false, opacity = 1, transparent = false, thickness = 0.05, toneMapped = true, angle = Math.PI, polygonOffset = false, polygonOffsetFactor = 0, renderOrder = 0, children, ref = $bindable(), ...props } = $props();
...
let geometry = $derived.by(() => {
    if (!isInstanceOf(parentMesh.current, 'Mesh'))
        return undefined;
