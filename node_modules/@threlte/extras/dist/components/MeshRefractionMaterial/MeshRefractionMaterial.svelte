<script lang="ts">import { isInstanceOf, T, useParent, useTask, useThrelte } from '@threlte/core';
...
let { envMap, bounces = 2, ior = 2.4, fresnel = 0, aberrationStrength = 0, color = 'white', fastChroma = true, ref = $bindable(), ...props } = $props();
const uniforms = {
    envMap: { value: null },
    bounces: { value: 2 },
    ior: { value: 2.4 },
...
onMount(() => {
    ...
    if ($parent && $parent instanceof Mesh && $parent.geometry) {
        ...
        uniforms.bvh.value.updateFrom(new MeshBVH($parent?.geometry.clone().toNonIndexed(), {
            strategy: SAH
        }));
    }
});
