<script lang="ts">import { MathUtils, Group } from 'three';
import { useTask, T } from '@threlte/core';
let { speed = 1, floatIntensity = 1, floatingRange = [-0.1, 0.1], rotationSpeed = 0, rotationIntensity = 0, seed = 10_000 * Math.random(), ref = $bindable(), children, ...props } = $props();
...
// Rotation
let rSpeed = $derived(Array.isArray(rotationSpeed) ? rotationSpeed : [rotationSpeed, rotationSpeed, rotationSpeed]);
