...
class VisualDiffReporter {
  ...
  onTestEnd(test, result) {
    ...
    const visualDiffAttachments = result.attachments.filter(
      attachment => attachment.name.includes('diff') || 
  ...
  generateMarkdownReport() {
    ...
    if (summary.failed > 0) {
      ...
      for (const test of failedTests) {
        ...
        const diffAttachments = test.attachments.filter(att => 
          att.name.includes('diff') || att.name.includes('expected') || att.name.includes('actual')
        );
