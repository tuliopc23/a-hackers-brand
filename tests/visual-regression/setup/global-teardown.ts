		...
		for (const reportFile of reportFiles) {
			...
			if (fs.existsSync(reportPath)) {
				try {
					...
					console.warn(
						`⚠️  Could not read report ${reportFile}: ${error instanceof Error ? error.message : String(error)}`
					);
				}
			}
		}
		...
		for (const tempFile of tempFiles) {
			if (fs.existsSync(tempFile)) {
				try {
					...
					console.warn(
						`⚠️  Could not delete temp file ${tempFile}: ${error instanceof Error ? error.message : String(error)}`
					);
				}
			}
		}
