...
async function globalSetup(config: FullConfig): Promise<void> {
	...
	try {
		...
		const componentPages = ['/components/button', '/components/input', '/components/card', '/components/modal'];
		...
		for (const componentPage of componentPages) {
			try {
				...
				console.warn(
					`⚠️  Could not warm up ${componentPage}: ${error instanceof Error ? error.message : String(error)}`
				);
			}
		}
		...
		for (const component of criticalComponents) {
			try {
				...
				console.warn(
					`⚠️  Could not generate baseline for ${component.name}: ${error instanceof Error ? error.message : String(error)}`
				);
			}
		}
	...
	fs.writeFileSync('test-results/visual-regression/setup-metadata.json', JSON.stringify(setupMetadata, null, 2));
}
