...
test.describe('Motion & Animation System Visual Tests', () => {
	...
	test('Spring pop animations', async ({ page }) => {
		...
		if (await springElements.count() > 0) {
			...
			if (await springElements.first().getAttribute('role') === 'button' || 
		...
		const interactiveSpring = page.locator('button, [role="button"]').filter({ has: page.locator('[data-spring="true"]') });
