...
async function generateComponent() {
  ...
  // Get options
  const includeTests = (await askQuestion(
    `${colors.cyan}Include test file? (y/n): ${colors.reset}`
  )).toLowerCase().startsWith('y');
  
  const includeStory = (await askQuestion(
    `${colors.cyan}Include story file? (y/n): ${colors.reset}`
  )).toLowerCase().startsWith('y');
  
  const includeTypes = (await askQuestion(
    `${colors.cyan}Include TypeScript definitions? (y/n): ${colors.reset}`
  )).toLowerCase().startsWith('y');
...
${Object.entries(templates).map(([key, desc]) => `  ${key.padEnd(12)} ${desc}`).join('\n')}

${colors.cyan}Categories:${colors.reset}
${Object.entries(categories).map(([key, path]) => `  ${key.padEnd(12)} ${path}`).join('\n')}
`);
  process.exit(0);
}
