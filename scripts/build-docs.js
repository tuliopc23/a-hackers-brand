...
async function buildDocs() {
	try {
		...
		const docFiles = files.filter((file) => file.endsWith('.svelte') || file.endsWith('.md') || file.endsWith('.svx'));
		...
		await writeFile(join(docsOutputDir, 'index.json'), JSON.stringify(index, null, 2));
