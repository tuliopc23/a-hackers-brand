...
<script lang="ts">import { DecalGeometry } from 'three/examples/jsm/geometries/DecalGeometry.js';
import { asyncWritable, T, useParent } from '@threlte/core';
import { useSuspense } from '../../suspense/useSuspense';
import { useTexture } from '../../hooks/useTexture';
let { src, mesh: parentMesh, position, rotation, scale, polygonOffsetFactor = -10, depthTest = true, debug = false, ref = $bindable(), children, ...rest } = $props();
const parent = useParent();
const parentNode = $derived(parentMesh ?? $parent);
const mesh = new Mesh();
...
const map = $derived(typeof src === 'string'
    ...
    : src
        ? asyncWritable(Promise.resolve(src))
        : undefined);
$effect.pre(() => {
    ...
    mesh.geometry = new DecalGeometry(parentNode, projectorPosition, projectorRotation, projectorSize);
