<script lang="ts">import { AsciiEffect } from 'three/examples/jsm/effects/AsciiEffect.js';
import { fromStore } from 'svelte/store';
import { useTask, useThrelte } from '@threlte/core';
const defaultCharacters = ' .:-+*=%@#';
let { autoRender = true, bgColor = '#000000', camera, characters = defaultCharacters, fgColor = '#ffffff', onstart, onstop, options = {}, scene, children } = $props();
const { autoRender: threlteAutoRender, camera: defaultCamera, renderStage, renderer, canvas, dom, scene: defaultScene, size } = useThrelte();
