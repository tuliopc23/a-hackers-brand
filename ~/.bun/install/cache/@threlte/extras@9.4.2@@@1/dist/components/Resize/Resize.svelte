...
<script lang="ts">import { isInstanceOf, observe, T, useStage, useTask, useThrelte } from '@threlte/core';
import { Box3, Group } from 'three';
import InjectPlugin from '../InjectPlugin/InjectPlugin.svelte';
const { renderStage } = useThrelte();
let { axis, auto = false, box = _box, precise, onresize, stage = useStage('<Resize>', { before: renderStage }), ref = $bindable(), children, ...props } = $props();
