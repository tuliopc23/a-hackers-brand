<script lang="ts">import { createSceneContext, observe, T, useTask, useThrelte } from '@threlte/core';
import { useCubeCamera } from '../../../hooks/useCubeCamera.svelte';
import { useEnvironment } from '../utils/useEnvironment.svelte';
const ctx = useThrelte();
let { far = 1000, frames = Infinity, isBackground = false, near = 0.1, onupdatestart, onupdatestop, resolution = 256, scene: parentScene = ctx.scene, visible, children } = $props();
// Create a parent scene to render the virtual environment into
const { scene } = createSceneContext();
export const { camera, renderTarget } = useCubeCamera(() => near, () => far, () => resolution);
useEnvironment({
