<script lang="ts">import { createCacheContext, createCameraContext, createDOMContext, createParentContext, createParentObject3DContext, createSceneContext, createUserContext, useTask, useThrelte } from '@threlte/core';
import { Vector4 } from 'three';
import { OffscreenObserver } from './OffscreenObserver.svelte';
let { dom, children } = $props();
...
const { start, stop } = useTask(Symbol('<View>'), () => {
