<script lang="ts">import { T, asyncWritable, isInstanceOf, useParent, useTask, useThrelte } from '@threlte/core';
import { Color, ShaderMaterial, Vector2, Vector3 } from 'three';
import { useTexture } from '../../hooks/useTexture';
import { useSuspense } from '../../suspense/useSuspense';
import { fragmentShader, vertexShader } from './shaders';
let { color = 'white', zoom = 1, radius = 0, alphaThreshold = 0, alphaSmoothing = 0.1, brightness = 0, contrast = 0, hue = 0, saturation = 0, lightness = 0, negative = false, opacity = 1, toneMapped = true, transparent = false, texture, monochromeColor, monochromeStrength, colorProcessingTexture, side, url, ref = $bindable(), children, ...props } = $props();
...
const textureStore = $derived(suspend(url ? useTexture(url) : asyncWritable(Promise.resolve(texture))));
let { size } = useThrelte();
