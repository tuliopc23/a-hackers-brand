...
<script lang="ts">import { useTask, useThrelte, useParent, observe } from '@threlte/core';
import { NoToneMapping, Vector4 } from 'three';
import { ViewportGizmo } from 'three-viewport-gizmo';
import { OrbitControls } from 'three/examples/jsm/controls/OrbitControls.js';
import { TrackballControls } from 'three/examples/jsm/controls/TrackballControls.js';
let { controls, renderTask, ref = $bindable(), onstart, onchange, onend, ...rest } = $props();
const parent = useParent();
const { camera, renderer, dom, autoRenderTask, shouldRender, size, invalidate } = useThrelte();
const gizmo = $derived.by(() => {
    invalidate();
    return new ViewportGizmo(camera.current, renderer, {
        container: dom,
...
const cameraControls = $derived(controls ?? $parent);
