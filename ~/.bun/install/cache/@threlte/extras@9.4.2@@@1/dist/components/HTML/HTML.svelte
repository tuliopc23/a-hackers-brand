...
<script lang="ts">import { T, useTask, useThrelte } from '@threlte/core';
import { DoubleSide, Group, Matrix4, Mesh, Object3D, OrthographicCamera, PerspectiveCamera, Raycaster, Vector3 } from 'three';
import { useSuspense } from '../../suspense/useSuspense';
import { logFragment, logVertex, spriteVertex } from './shaders';
import { defaultCalculatePosition, epsilon, getCameraCSSMatrix, getObjectCSSMatrix, getViewportFactor, isObjectBehindCamera, isObjectVisible, objectScale, objectZIndex } from './utils';
let { autoRender = true, eps = 0.001, center = false, fullscreen = false, portal, distanceFactor, sprite = false, transform = false, occlude = false, castShadow = false, receiveShadow = false, material, geometry, zIndexRange = [16777271, 0], calculatePosition = defaultCalculatePosition, as = 'div', wrapperClass, pointerEvents = 'auto', ref = $bindable(), visible = $bindable(), children, ...props } = $props();
visible = true;
...
let oldZoom = 0;
let oldPosition = [0, 0];
let transformOuterRef = $state();
let transformInnerRef = $state();
let isMeshSizeSet = false;
const occlusionMesh = new Mesh();
const raycaster = new Raycaster();
let isRayCastOcclusion = $derived((occlude && occlude !== 'blending') || (Array.isArray(occlude) && occlude.length > 0));
