<script lang="ts">
	...
	let mounted = false;
	import { threlteConfig, threlteActions } from '$lib/stores/threlte.js';

	// Use store state with $derived
	let show3D = $derived($threlteConfig.enabled);
	let sceneType = $derived($threlteConfig.scene);
	let quality = $derived($threlteConfig.quality);
	let autoRotate = $derived($threlteConfig.autoRotate);
	let renderInfo = $derived($threlteConfig.performance);
