...
// Temporary Card, Button, Input components until we integrate with glass-ui
const Card = ({ children, className, variant, ...props }: any) => (
  <div className={cn('rounded-lg border shadow-sm', variant === 'glass' && 'bg-white/10 border-white/20 backdrop-blur-md', className)} {...props}>
    {children}
  </div>
);

const Button = ({ children, className, variant, size, ...props }: any) => (
  <button className={cn('inline-flex items-center justify-center rounded-md font-medium transition-colors', className)} {...props}>
    {children}
  </button>
);

const Input = ({ className, ...props }: any) => (
  <input className={cn('flex w-full rounded-md border border-input bg-background px-3 py-2 text-sm', className)} {...props} />
);
...
export function TerminalWindow({
  ...
  const playSound = useCallback((type: 'keypress' | 'enter' | 'error') => {
    ...
    // Simple audio feedback (in a real implementation, you'd load actual sound files)
