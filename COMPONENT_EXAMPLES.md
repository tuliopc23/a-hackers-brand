...
<!-- With glow effects -->
<Button variant="liquid" glow={true}>
  Glowing Button
</Button>
...
<!-- As link -->
<Button href="/dashboard" variant="terminal">
  Go to Dashboard
</Button>
```
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
  <!-- Feature cards -->
  <Card variant="glass" class="p-6">
    <div class="w-12 h-12 bg-cyan-500/20 rounded-lg mb-4 flex items-center justify-center">
      🚀
    </div>
...
<Tooltip 
  text="System information: CPU 45%, RAM 67%" 
  variant="terminal"
  position="bottom"
  delay={200}
>
...
<Tooltip 
  text="Advanced liquid glass effects with smart positioning"
  variant="liquid"
  position="right"
  arrow={true}
>
...
<Card variant="glass" class="max-w-md mx-auto p-6">
  ...
  {#if submitSuccess}
    <Alert 
      ...
      description="Welcome to A Hacker's Brand"
    />
  {:else}
    ...
    <div class="space-y-2">
      {#each paginatedUsers as user}
        <div class="flex items-center gap-4 p-4 border border-white/10 rounded-lg hover:bg-white/5 transition-colors">
          ...
          <div class="text-sm">
            <span class="inline-block px-2 py-1 rounded text-xs {user.status === 'active' ? 'bg-green-500/20 text-green-400' : 'bg-gray-500/20 text-gray-400'}">
              {user.status}
            </span>
          </div>
  ...
  <div class="max-w-7xl mx-auto">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <Card variant="glass" class="lg:col-span-2 p-6">
        ...
        {:else}
          <div class="h-80 bg-gradient-to-br from-cyan-500/10 to-blue-500/10 rounded-lg border border-white/10 flex items-center justify-center">
...
<Card variant="liquid" class="p-6 mt-6">
  <div 
    ...
    class="p-4 bg-white/10 rounded-lg cursor-pointer"
  >
...
<div class="mt-6">
  <Button 
    use:springPop={{ scale: 0.95, duration: 200 }}
    variant="neon"
  >
    Spring Pop Button
  </Button>
