<script lang="ts">
    ...
    const currentVariant = variants[variant]; // Cleanup: removed __markUsed scaffold and no-op $effect

    let bootContainer: HTMLDivElement;
    let displayedMessages = $state<
        Array<{ text: string; type?: BootMessage["type"] }>
    >([]);

    function getMessageClass(t?: BootMessage["type"]) {
        return t ? messageTypes[t] : currentVariant.text;
    }
    ...
    export const start = () => startBootSequence();
</script>
...
>
    ...
    <div
        class="absolute inset-0 pointer-events-none border-4 border-current opacity-10 rounded-lg"
    ></div>
    ...
    <div class="relative z-10 space-y-1">
        ...
        {#if isBooting && currentText}
            <div
                class={cn(
                    "whitespace-pre-wrap",
                    getMessageClass(bootMessages[currentMessageIndex]?.type),
                )}
            >
                {currentText}
