...
>
	...
	<!-- Progress Bar -->
	{#if showProgress}
		<div class={cn('absolute bottom-0 left-0 right-0 h-1', currentVariant.progress)}>
			<div
				class={cn('h-full transition-all duration-300', currentVariant.progressFill)}
			...
			{#each items() as _, index (index)}
				<button
					...
					class={cn(
						...
						currentSize.dots,
						index === currentIndex ? currentVariant.dotsActive : currentVariant.dots
					)}
