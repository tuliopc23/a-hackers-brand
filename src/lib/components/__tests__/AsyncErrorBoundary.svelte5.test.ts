...
describe('AsyncErrorBoundary Component (Svelte 5)', () => {
	...
	describe('CSS Class Generation', () => {
		it('should generate correct error container classes', () => {
			...
			expectClassesToContain(container, ['p-6', 'rounded-lg', 'text-center', 'glass-medium']);
		});
		...
		it('should generate correct loading container classes', () => {
			...
			simulateClassGeneration(container, 'flex items-center justify-center p-8', variant, '', ['glass-subtle']);

			expectClassesToContain(container, ['flex', 'items-center', 'justify-center', 'p-8']);
		});
	...
	describe('Performance', () => {
		...
		it('should handle concurrent promises', () => {
			const promises = [Promise.resolve('result1'), Promise.resolve('result2'), Promise.reject(new Error('error'))];
