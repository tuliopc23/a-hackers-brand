...
describe('TerminalWindow Component (Svelte 5)', () => {
	...
	describe('CSS Class Generation', () => {
		it('should generate correct window container classes', () => {
			...
			expectClassesToContain(container, ['rounded-lg', 'overflow-hidden', 'bg-gray-900', 'font-mono']);
		});
		...
		it('should generate correct header classes', () => {
			...
			expectClassesToContain(header, ['flex', 'items-center', 'justify-between', 'bg-gray-800']);
		});
	...
	describe('Typewriter Effect', () => {
		...
		it('should respect typing speed', () => {
			...
			Object.entries(speeds).forEach(([name, delay]) => {
				...
				if (name === 'slow') expect(typeDelay).toBe(100);
				if (name === 'normal') expect(typeDelay).toBe(50);
				if (name === 'fast') expect(typeDelay).toBe(25);
			});
		});
	...
	describe('Command Processing', () => {
		...
		it('should support command aliases', () => {
			...
			const command = 'll';
			const expandedCommand = aliases[command as keyof typeof aliases] || command;
