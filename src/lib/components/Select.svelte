<script lang="ts">
    ...
    function navigateOptions(direction: number) {
        ...
        activeIndex = options.findIndex(
            (opt) => opt === enabledOptions[newIndex],
        );
    }
...
<div
    ...
    aria-activedescendant={activeIndex > -1
        ? `${uniqueId}-option-${activeIndex}`
        : undefined}
    tabindex={disabled ? -1 : 0}
    use:magneticHover={(magnetic || jelly) && !disabled && !reduceMotion
        ? { strength: magnetic ? 0.1 : 0.08, distance: magnetic ? 40 : 20 }
        : undefined}
...
{#if isOpen}
    <ul
        ...
        out:glassFade={{
            direction: "up",
            distance: 10,
            duration: animate && !reduceMotion ? 100 : 0,
        }}
    >
