<script lang="ts">
    ...
    // Cleanup: removed previous __markUsed scaffold and no-op $effect.
    // Template + bind:this usage is sufficient; unnecessary artificial references eliminated.
    ...
    // Removed duplicate currentVariant/currentSize declarations (already declared above)
...
{#if open}
    ...
    >
        ...
        >
            ...
            >
                <div class="flex-1">
                    {#if typeof children === "object" && children && "header" in children && children.header}
                        {@const __headerSnippet = children.header()}
                        {@render __headerSnippet()}
                    {:else}
                ...
                {#if showCloseButton && !persistent}
                    <button
                        onclick={closeDrawer}
                        class={cn(
                            "p-2 rounded-lg border transition-colors ml-4",
            ...
            <div class="flex-1 overflow-y-auto p-4">
                {#if typeof children === "function"}
                    {@const __bodySnippet = children({
                        open,
                        close: closeDrawer,
                    })}
                    {@render __bodySnippet()}
                {:else if typeof children === "object" && children && "body" in children && children.body}
                    {@const __bodySnippet = children.body({
                        open,
                        close: closeDrawer,
                    })}
                    {@render __bodySnippet()}
            ...
            <!-- Footer -->
            {#if typeof children === "object" && children && "footer" in children && children.footer}
                {@const __footerSnippet = children.footer({
                    open,
                    close: closeDrawer,
                })}
                <div class="p-4 border-t border-white/10 flex-shrink-0">
                    {@render __footerSnippet()}
                </div>
            {/if}
        </div>
    </div>
