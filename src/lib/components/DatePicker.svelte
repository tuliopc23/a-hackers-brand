<script lang="ts">
    ...
    function parseDate(str: string): Date | null {
        ...
        // Cross-check that the constructed date matches the components (guards invalid like 2024-02-31)
        if (
    ...
    // Stable derived calendar days (prevents regenerating array on each render call)
    ...
    $effect(() => {
        if (value) {
            const date =
                value instanceof Date ? value : parseDate(value.toString());
            if (date) {
                selectedDate = date;
                inputValue = formatDate(date);
...
<div class="relative">
    ...
    <div class="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none">
        <svg
            class="w-5 h-5 {currentVariant.header}"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
        >
    ...
    {#if isOpen}
        ...
        >
            ...
            <div class="flex items-center justify-between mb-4">
                ...
                <h3
                    class="text-lg font-semibold {currentVariant.header} font-mono"
                    aria-live="polite"
                >
            ...
            <div class="grid grid-cols-7 gap-1 mb-2" role="row">
                {#each dayNames as day (day)}
                    <div
                        class="p-2 text-center text-xs font-semibold {currentVariant.header} font-mono"
                        role="columnheader"
                    >
                        {day}
                    </div>
            ...
            >
                {#each calendarDays as day, i (day ? day.toDateString() : "blank-" + i)}
                    {#if day}
                        ...
                        <button
                            ...
                            tabindex={isSelected || isSameDate(day, focusedDate)
                                ? 0
                                : -1}
                            use:magneticHover={{
                                enabled: isValid,
