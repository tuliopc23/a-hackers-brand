<script lang="ts">
    ...
    // Removed unused motion imports (glassFade, magneticHover) to satisfy analyzer
    ...
    const isAllSelected = $derived(
        items.length > 0 &&
            items
                ...
                .every((item) => currentSelectedItems.has(item.id)),
    );
    ...
    // (Cleanup) Removed legacy __markUsed scaffold that artificially referenced symbols.

    // (Cleanup) Removed no-op $effect previously used only to silence analyzer warnings.
</script>
        ...
        <div class="px-4 py-3 border-b {currentVariant.item} border-gray-600">
            <label class="flex items-center gap-3 cursor-pointer">
                ...
                <span class="text-sm font-medium">
                    {currentSelectedItems.size} of {items.filter(
                        (item) => !item.disabled,
                    ).length} selected
                </span>
            </label>
        ...
        {#if loading}
            ...
            {#each skeletonItems as skeleton, index (skeleton.id)}
                ...
                >
                    ...
                    {:else}
                        <div
                            class="border rounded-lg p-4 space-y-3 {currentVariant.skeleton}"
                        >
                            <div class="flex items-center gap-3">
                                <div
                                    class={cn(
...
{#each Object.entries(item.metadata) as [key, value] (key)}
                    ...
                    {:else}
                        <!-- Card Layout -->
                        <div
                            class="border rounded-lg p-4 {currentVariant.item}"
                        >
                            <div class="flex items-start gap-3">
                                {#if selectable}
                                    <input
