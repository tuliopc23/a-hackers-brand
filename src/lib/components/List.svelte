			...
			{#each items() as item, index (item.id)}
				...
				>
					{#if layout === 'simple'}
						...
						<div class="flex items-center gap-3">
							{#if selectable}
								<input
									...
									onclick={(e) => e.stopPropagation()} onkeydown={(e) => e.key === "Enter" && handleAction()} 
									class="rounded bg-transparent border-current"
								/>
							{/if}
                      ...
                      onclick={(e) => handleActionClick(item, action.action, e)} onkeydown={(e) => e.key === "Enter" && handleActionClick(item, action.action, e)}
