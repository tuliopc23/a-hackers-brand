<script lang="ts">
	import { CommandBlock } from '$lib/components';
	...
	const componentMeta = {
		name: 'CommandBlock',
		description: 'Code block styled as terminal output',
		category: 'terminal',
		complexity: 'simple',
		since: '0.2.0'
	};
	...
	const basicExample = `<CommandBlock />`;
	
	const advancedExample = `<script>
  import { CommandBlock } from 'a-hackers-brand';
