<script lang="ts">
	import { BootSequence } from '$lib/components';
	...
	const componentMeta = {
		name: 'BootSequence',
		description: 'System boot animation sequence',
		category: 'terminal',
		complexity: 'complex',
		since: '0.2.0'
	};
	...
	const basicExample = `<BootSequence />`;
	
	const advancedExample = `<script>
  import { BootSequence } from 'a-hackers-brand';
