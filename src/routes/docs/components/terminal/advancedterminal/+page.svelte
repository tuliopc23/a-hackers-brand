<script lang="ts">
	import { AdvancedTerminal } from '$lib/components';
	...
	const componentMeta = {
		name: 'AdvancedTerminal',
		description: 'Full terminal emulator with command execution',
		category: 'terminal',
		complexity: 'complex',
		since: '0.2.0'
	};
	...
	const basicExample = `<AdvancedTerminal />`;
	
	const advancedExample = `<script>
  import { AdvancedTerminal } from 'a-hackers-brand';
...
<div class="prose prose-invert max-w-none">
	...
	{#if playgroundProps.length > 0}
		<section class="mb-12">
			...
			<ComponentPlayground 
				componentName="AdvancedTerminal"
				component={AdvancedTerminal} 
	...
	<section>
		...
		<div class="flex flex-wrap gap-2">
			<a href="/docs/components/terminal" class="text-green-400 hover:underline">
				View all terminal components →
			</a>
		</div>
	</section>
</div>